...
<?php if($data[0]!=NULL){ ?>
    ...
    var lonLat = new OpenLayers.LonLat( 111.9327785 ,-7.6588644 ).transform(epsg4326, projectTo);
    ...
    semua = String("<p style='font-size:15px;line-height:20px;margin-bottom:7px'><strong>WS :</strong> <?php echo $row['nama'];?><br>"
    +"</p>");

    var feature = new OpenLayers.Feature.Vector(
            new OpenLayers.Geometry.Point( <?php echo $row['x']?> ,<?php echo $row['y']?> ).transform(epsg4326, projectTo),
            {description:semua} ,
            {externalGraphic: '<?php echo base_url();echo 'assets/img/icon-maps.png'?>', graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25  }
        );
...
function validateText2(id)
{
  if($("#"+id).val()==null || $("#"+id).val()=="")
  {
    ...
    div.addClass("has-error has-feedback");
    div.append('<span id="glypcn'+id+'" class="glyphicon glyphicon-remove form-control-feedback"></span>');
    $('.nama-validate').removeClass("hide");
    return false;
  }
  else{
    var div = $("#"+id).closest("div");
...
$(document).ready(
  function()
  {
    ...
    {
      if(validateText2("contactNama")&&validateText("contactInstansi")&&validateEmail("contactEmail"))
      {

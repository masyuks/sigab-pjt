...
<script>
    ...
    +"<strong>Curah Hujan :</strong> <?php echo $row['hourly_rf']?> mm<br>"
    +"<strong>Kumulatif :</strong> <?php echo $row['cumulative']?> mm<br>"
    +"<strong>Siaga Siaga :</strong> <?php echo $row['siaga']?><br>"
    ...
    var feature = new OpenLayers.Feature.Vector(
            ...
            {description:semua} ,
            <?php if($row['siaga']=='NORMAL') { if($row['hourly_rf']==0 ){ ?>
...
$(".table-toogle").click(function(){
  ...
  $('.div-home-2').toggleClass("display-show display-hide");
    ...
    function peta_detail(encrypt){
      console.log(encrypt);
      window.location = "<?php  echo base_url('index.php/home/home_detail_arr/'); ?>"+encrypt;
    }
    function search_func_arr(val){
      if(val=='')
      {alert('data pencarian tidak boleh kosong');}
      else{
        val = val.replace(/ /g,'-');
        window.location = "<?php  echo base_url('index.php/home/search_arr/'); ?>"+val;
      }
    }
    ...
    $(document).ready(
      function()
      {
        $(".search-daerah").keyup(function(event) {
            if (event.keyCode === 13) {
      search_func_arr($('.search-daerah').val());
            }
        });
        $(".search-daerah-2").keyup(function(event) {
            if (event.keyCode === 13) {
      search_func_arr($('.search-daerah-2').val());
            }
        });
    }
